...
function handleAction($action) {
    switch ($action) {
        ...
        // Admin-native job vacancy management (uses main admin session — no HR panel dependency)
        case 'jobs_list':
            return adminJobsList();
        case 'jobs_save':
            return adminJobsSave();
        case 'jobs_delete':
            return adminJobsDelete();
        case 'jobs_toggle_status':
            return adminJobsToggleStatus();
...
// ============ ADMIN-NATIVE JOB VACANCY MODULE ============
// Clean implementation using the main admin session only.
// No HR-panel authentication dependency.
...
function adminJobsList() {
    ...
    $db = requireDb(['job_listings']);
    if (is_array($db)) return ['success' => false, 'error' => $db['error']];
    $rows = dbAll("SELECT * FROM job_listings ORDER BY created_at DESC", []);
    return ['success' => true, 'data' => array_map(static fn($r) => normalizeJobListingRow($r, false), $rows)];
}

function adminJobsSave() {
    checkAuthentication();
    ensureJobListingsSchema();
    $data = requestData();
    ...
    $result = dbUpsert(
        'job_listings',
        ['id','title','department','location','type','description','requirements','salary_range','show_salary_range','closing_date','status','applications_count','created_at','updated_at','created_by'],
        $data, 'job'
    );
    if (!empty($result['success'])) {
        exportPublishedJobsCache();
...
function adminJobsDelete() {
    checkAuthentication();
    ensureJobListingsSchema();
    $id = $_GET['id'] ?? ($_POST['id'] ?? '');
    if (!$id) return ['success' => false, 'error' => 'Vacancy ID required'];
    dbWrite("DELETE FROM job_listings WHERE id = ?", [$id]);
    exportPublishedJobsCache();
    logActivityDb('jobs_delete', $id);
    return ['success' => true, 'message' => 'Vacancy deleted'];
}

function adminJobsToggleStatus() {
    checkAuthentication();
    ensureJobListingsSchema();
    $id  = $_GET['id'] ?? ($_POST['id'] ?? '');
    $job = dbOne("SELECT status FROM job_listings WHERE id = ?", [$id]);
    ...
    dbWrite("UPDATE job_listings SET status = ?, updated_at = NOW() WHERE id = ?", [$next, $id]);
    exportPublishedJobsCache();
    logActivityDb('jobs_toggle_status', "$id → $next");
    return ['success' => true, 'message' => 'Status updated', 'status' => $next];
}

// ============ END ADMIN-NATIVE JOB VACANCY MODULE ============
