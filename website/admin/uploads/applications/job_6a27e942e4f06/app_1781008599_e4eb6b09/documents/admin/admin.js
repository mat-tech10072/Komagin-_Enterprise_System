    ...
    displayProjects() {
        ...
        container.innerHTML = `<div class="project-card-list">${this.filteredProjects.map(project => {
            ...
            const statusBadge = project.status === 'COMPLETED'
                ...
                : `<span class="project-row-badge" style="background:rgba(201,162,39,0.12);color:#A68318;border:1px solid rgba(201,162,39,0.25)"><i class="fas fa-clock"></i> Pending</span>`;
...
(function() {
    ...
    KomaginAdmin.prototype.renderDashboardCharts = function() {
        ...
        const volumeData = [
            { label: 'Contacts', value: contacts.length, color: dashboardPalette[0] },
            { label: 'Subscribers', value: subscribers.length, color: dashboardPalette[1] },
            { label: 'Open Jobs', value: jobs.filter(item => String(item.status || '') === 'published').length, color: dashboardPalette[2] },
            { label: 'Applications', value: applications.length, color: dashboardPalette[3] }
        ];
        ...
        const publishingStatusData = [
            ...
            { label: 'Live Documents', value: documents.filter(item => Number(item.is_active || 0) === 1).length, color: dashboardPalette[1] },
...
(function() {
    ...
    KomaginAdmin.prototype.loadJobs = async function() {
        ...
        try {
            const result = await this.readAdminJsonResponse(
                fetch(`${this.apiBase}?action=hr_get_jobs`),
    ...
    KomaginAdmin.prototype.saveJob = async function(formData) {
        ...
        try {
            ...
            const result = await this.readAdminJsonResponse(
                fetch(`${this.apiBase}?action=hr_save_job`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(payload)
                }),
    ...
    KomaginAdmin.prototype.deleteJob = async function(id) {
        this.showStyledConfirm({
            ...
            onConfirm: async () => {
                ...
                try {
                    const result = await this.readAdminJsonResponse(
                        fetch(`${this.apiBase}?action=hr_delete_job&id=${encodeURIComponent(id)}`),
    ...
    KomaginAdmin.prototype.toggleJobStatus = async function(id) {
        ...
        try {
            const result = await this.readAdminJsonResponse(
                fetch(`${this.apiBase}?action=hr_toggle_job_status&id=${encodeURIComponent(id)}`),
...
(function() {
    ...
    KomaginAdmin.prototype.refreshDashboardDataFeeds = async function() {
        const requests = [
            fetch(`${this.apiBase}?action=hr_get_jobs`).then(r => r.json()).catch(() => ({ success: false, data: [] })),
            fetch(`${this.apiBase}?action=documents_get_all`).then(r => r.json()).catch(() => ({ success: false, data: [] })),
            fetch(`${this.apiBase}?action=hire_items_get_all`).then(r => r.json()).catch(() => ({ success: false, data: [] })),
            fetch(`${this.apiBase}?action=hr_get_applications&status=all`).then(r => r.json()).catch(() => ({ success: false, data: [] }))
        ];
        const [jobsResult, documentsResult, hireResult, applicationsResult] = await Promise.all(requests);
        this.dashboardJobs = jobsResult.success ? (jobsResult.data || []) : (this.dashboardJobs || []);
        this.dashboardDocuments = documentsResult.success ? (documentsResult.data || []) : (this.dashboardDocuments || []);
        this.dashboardHireItems = hireResult.success ? (hireResult.data || []) : (this.dashboardHireItems || []);
        this.dashboardApplications = applicationsResult.success ? (applicationsResult.data || []) : (this.dashboardApplications || []);
    ...
    KomaginAdmin.prototype.renderDashboardCharts = function() {
        ...
        const volumeData = [
            { label: 'Contacts', value: contacts.length, color: palette[0] },
            { label: 'Subscribers', value: subscribers.length, color: palette[1] },
            { label: 'Open Jobs', value: jobs.filter(item => String(item.status || '') === 'published').length, color: palette[2] },
            { label: 'Applications', value: applications.length, color: palette[3] }
        ];
        ...
        const publishingStatusData = [
            ...
            { label: 'Live Documents', value: documents.filter(item => Number(item.is_active || 0) === 1).length, color: palette[1] },
...
(function() {
    ...
    KomaginAdmin.prototype.refreshDashboardDataFeeds = async function() {
        ...
        const [jobsResult, documentsResult, hireResult, applicationsResult] = await Promise.all([
            safeRead(`${this.apiBase}?action=hr_get_jobs`),
            safeRead(`${this.apiBase}?action=documents_get_all`),
            safeRead(`${this.apiBase}?action=hire_items_get_all`),
            safeRead(`${this.apiBase}?action=hr_get_applications&status=all`)
